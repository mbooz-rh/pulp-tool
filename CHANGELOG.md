...
- **`AGENTS.md`:** canonical agent entry with § **Bootstrap** (read-first order to reduce context thrash); pointers to `docs/ARCHITECTURE.md`, `CLAUDE.md`, and optional `@llm-development-guidelines-deep`
- **`.cursor/rules/llm-development-guidelines-deep.mdc`:** full LLM dev reference (lint detail, PR templates, troubleshooting); `alwaysApply: false`, use `@llm-development-guidelines-deep` when needed
...
### Changed
- **`.cursor/rules/llm-development-guidelines.mdc`:** slim always-on essentials (workflow, diff coverage, PR/CHANGELOG rules); lengthy content moved to `llm-development-guidelines-deep.mdc`
