    ...
    def repository_operation(
        ...
        *,
        ...
        if operation == "create_repo":
            ...
            return self._create_repository(endpoint, repository_data)  # type: ignore[arg-type]
        if operation == "get_repo":
            endpoint = f"api/v3/repositories/{repo_type}/{repo_type}/"
            return self._get_single_resource(endpoint, name)  # type: ignore[arg-type]
        if operation == "create_distro":
            endpoint = f"api/v3/distributions/{repo_type}/{repo_type}/"
            return self._create_distribution(endpoint, distribution_data)  # type: ignore[arg-type]
