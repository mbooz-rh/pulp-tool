...
@click.option(
    "--distribution-name",
    help="A unique name for this repository's distribution. (required when not using json)",
)
@click.option(
    "--base-path",
...
def create_repository(  # pylint: disable=too-many-arguments,too-many-positional-arguments
    ...
    if json_data:
        ...
        if distribution_name is None:
            logging.error("Missing option --distribution-name: Input should be a valid string")
            missing_option = True
        ...
        try:
            repo_data = CreateRepository(
                ...
                distribution_options=DistributionOptions(
                    name=distribution_name, base_path=base_path, generate_repo_config=generate_repo_config
                ),
            )
        except ValidationError as e:
